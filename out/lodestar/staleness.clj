...
(defn time-stale [idx ^String today before?]
  ...
  (if (and (not (k/terminal-i? idx te)) (and (some? vu) (before? vu today))) (conj acc (->Review te "valid_until" (str vu " is past " today " — re-validate or drop"))) acc))) [] (k/thread-ids-i idx)))

(defn edge-stale [idx]
  (reduce (fn [acc te] (if (k/terminal-i? idx te) acc (reduce (fn [a p] (reduce (fn [b tgt] (if (some? (k/one-i idx tgt "abandoned")) (conj b (->Review te p (str "→ " tgt " was abandoned — relationship may be stale"))) b)) a (k/many-i idx te p))) acc edge-preds))) [] (k/thread-ids-i idx)))
...
(defn estimate-stale [idx latest]
  (reduce (fn [acc v] (if (and (= (:p v) "estimate_hours") (and (not (k/terminal-i? idx (:l v))) (> (later-edit-scope-tx latest (:l v)) (:tx v)))) (conj acc (->Review (:l v) "estimate_hours" (str (:r v) "h estimated before a later scope edit — re-estimate"))) acc)) [] latest))
...
(defn promotable [idx]
  (filterv (fn [te] (and (nil? (k/one-i idx te "committed")) (and (not (k/terminal-i? idx te)) (has-structure? idx te)))) (k/thread-ids-i idx)))
